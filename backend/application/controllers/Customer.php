...
class Customer extends CI_Controller {
    ...
    public function __construct() {
        ...
        // CORS Headers - IMPORTANT: Adjust 'Access-Control-Allow-Origin' for production
        header('Access-Control-Allow-Origin: http://localhost:4200'); // Your Angular app's origin
    ...
    /**
     * Existing get_customer_details method (if you have it)
     * Retrieves user details for autofill.
     */
    public function details($user_id) {
        $user_details = $this->User_model->get_user_details_by_id($user_id);

        if ($user_details) {
            unset($user_details['password']); // Never send password hash
            $this->output
    ...
    /**
     * API endpoint to get all saved addresses for a user.
     * Requires a user_id.
     * GET: /customer/addresses/{user_id}
     */
    ...
    // ... (any other methods in your Customer controller) ...
}